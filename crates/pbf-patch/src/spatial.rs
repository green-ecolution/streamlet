...
mod tests {
    ...
    fn test_network() -> (Vec<Way>, HashMap<NodeId, Coordinate>) {
        let nodes: HashMap<NodeId, Coordinate> = [
            ...
            (5, Coordinate { lat: 54.8100, lon: 9.4700 }),
            (6, Coordinate { lat: 54.8100, lon: 9.4710 }),
        ]
        .into();
        ...
        (vec![way(100, &[1, 2, 3]), way(200, &[3, 4]), way(300, &[5, 6])], nodes)
    }
    ...
    fn matches_nearest_way_within_threshold() {
        ...
        let (way_id, dist) = index
            .match_point(Coordinate { lat: 54.79505, lon: 9.4305 })
            .unwrap();
    ...
    fn no_match_beyond_threshold() {
        let (ways, nodes) = test_network();
        let index = WayIndex::build(&ways, &nodes);
        assert_eq!(index.match_point(Coordinate { lat: 54.90, lon: 9.60 }), None);
    }

    #[test]
    fn ways_with_fewer_than_two_known_nodes_are_skipped() {
        let nodes: HashMap<NodeId, Coordinate> =
            [(1, Coordinate { lat: 54.7950, lon: 9.4300 })].into();
        let ways = vec![Way {
            id: 100,
            version: 1,
            node_refs: vec![1, 99],
        ...
        let index = WayIndex::build(&ways, &nodes);
        assert_eq!(index.match_point(Coordinate { lat: 54.7950, lon: 9.4300 }), None);
    }
}
